...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
    ...
    float b = pow(sin(coords.y * 3.141), 4);
    float opacity = (s + 0.5) * b;
    ...
    float4 tColor = mixedSample * finalColor * input.Color;
    float4 colorCorrected = lerp(color, float4(tColor.xyz, opacity), opacity);
